...
> **兼容性**：v0.7.5 支持 DSH **0.1.2-alpha.4+**，已在 **0.1.5-rc.1** 实测（真实-Cordis 探针通过；peer `@deepseek-ai/cordis ^4.0.2`）。历史读取同时兼容旧版 `session.events` 与 0.1.5+ 的 `snapshotEvents()`，**旧版 DSH 仍可用**。收到过「重启后所有通知静默」问题的请升级到 **0.7.4+**（见 FAQ）。
...
- **重启 DSH 后所有通知都没了？** 这是 **0.7.3 及更早**的已知缺陷：子代理判定当时写作 `delegationDepth !== undefined`，会把每次 `dsh web` 重启后带 `delegationDepth: 0` 的**主会话**误判为子代理并静默过滤。**请升级到 0.7.4 及以上**（判定已改为 `(delegationDepth ?? 0) > 0`）：执行 `dsh plugin --profile web update dsh-notify-windows` 后重启 DSH 宿主。
- **升级 DSH 到 0.1.5+ 后摘要变空、/goal 中间回合开始提醒、`never` 策略仍提醒？** DSH 0.1.5 移除了 `session.events` 属性（改用 `snapshotEvents()`），**0.7.4 及更早**读取该属性会拿到 `undefined` 后静默降级：通知摘要为空、`/goal` 轮次识别失效、审批策略读不到。**请升级到 0.7.5 及以上**（历史读取已同时兼容 `session.events` 与 `snapshotEvents()`，旧版 DSH 不受影响）。
