 ...
 *
 * DSH 0.1.5 removed the public `session.events` property; history is now read
 * on demand through `snapshotEvents()`, which returns `readonly SessionEvent[]`.
 * Prefer the legacy property so older DSH stays supported, then fall back to
 * the new accessor.
 *
...
export function sessionEvents(session) {
  ...
  try {
    ...
    // never break the notification path on a host-side failure
  }
  return [];
}
